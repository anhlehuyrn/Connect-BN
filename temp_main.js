const GlobalState = { score: 0, fillingReady: false, batterReady: false, wrapped: false, steamed: false, language: 'vi' }
const I18N = {
  vi: {
    title: 'B谩nh Phu Th锚',
    loading: '膼ang t岷 demo鈥?,
    nhanHeader: 'Nh芒n: K茅o nguy锚n li峄噓 v脿o ch岷 r峄搃 b岷 Khu岷',
    stir: 'Khu岷',
    next: 'Ti岷縫 t峄',
    voHeader: 'V峄? B岷 Khu岷 nhi峄乽 l岷 膽岷縩 khi 膽岷 膽峄?m峄媙',
    mix: 'Khu岷',
    goiHeader: 'G贸i: Ch峄峮 l谩 chu峄慽 膽煤ng, 膽岷穞 b峄檛 v脿o gi峄痑 r峄搃 b岷 G贸i',
    wrap: 'G贸i',
    hapHeader: 'H岷: Gi峄?n煤t H岷 膽峄?n岷 b谩nh',
    steam: 'H岷',
    endCongrats: 'B谩nh phu th锚 膽茫 h岷 ch铆n! Ch煤c m峄玭g!',
    endTip: 'B谩nh phu th锚 t瓢峄g tr瓢ng cho t矛nh ngh末a v峄?ch峄搉g鈥?,
    replay: 'Ch啤i l岷',
    missing: (list) => 'Thi岷縰: ' + list.join(', '),
    readyStir: '膼茫 膽峄?nguy锚n li峄噓, b岷 Khu岷',
    wrongLeaf: 'L谩 kh么ng 膽煤ng, h茫y ch峄峮 l谩 chu峄慽',
  },
  en: {
    title: 'Phu The Cake',
    loading: 'Loading demo鈥?,
    nhanHeader: 'Filling: Drag ingredients into the pan, then press Stir',
    stir: 'Stir',
    next: 'Next',
    voHeader: 'Dough: Click Mix repeatedly to reach consistency',
    mix: 'Mix',
    goiHeader: 'Wrap: Choose correct banana leaf, place dough, then Wrap',
    wrap: 'Wrap',
    hapHeader: 'Steam: Hold the Steam button to cook',
    steam: 'Steam',
    endCongrats: 'Phu The cake is cooked! Congrats!',
    endTip: 'Phu The cake symbolizes marital harmony鈥?,
    replay: 'Replay',
    missing: (list) => 'Missing: ' + list.join(', '),
    readyStir: 'All ingredients ready. Press Stir',
    wrongLeaf: 'Incorrect leaf. Choose banana leaf',
  }
}
function t(key, ...args) { const dict = I18N[GlobalState.language]; const val = dict[key]; return typeof val === 'function' ? val(...args) : val }
...
class SceneNhan extends Phaser.Scene {
  ...
  create() {
    ...
    const ingredients = [
      { key: 'dau', label: '膼岷璾 xanh', x: 120, y: 160 },
      { key: 'duong', label: '膼瓢峄漬g', x: 120, y: 280 },
      { key: 'dua', label: 'D峄玜 n岷', x: 120, y: 400 },
    ]
    const added = new Set()
